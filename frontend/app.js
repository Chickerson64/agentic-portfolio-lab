...
function renderOverview(state) {
  ...
  const controls = state.health.persisted ? `<article class="surface"><span class="kicker">Weekly funding</span><h2>Paired Cash Event</h2><form id="cash-event-form" class="command-form"><input name="amount" inputmode="decimal" value="1000" aria-label="Cash amount"><input name="currency" value="USD" aria-label="Currency"><input name="source" value="manual deposit" aria-label="Source"><input name="effective_at" type="datetime-local" aria-label="Effective time" required><button class="refresh-button" type="submit">Apply Cash Event</button></form><p class="muted">Managed cash ${esc(money(portfolio.cashValue, portfolio.baseCurrency))} · Benchmark cash ${esc(money(benchmark.snapshot.cashValue, benchmark.snapshot.baseCurrency))}</p><p class="muted">Benchmark status: ${esc(state.dashboard.benchmarkFulfillmentStatus)}</p><input id="benchmark-fulfilled-at" type="datetime-local" aria-label="Benchmark fulfillment time" required><button class="quiet-button" id="fulfill-benchmark" type="button">Fulfill SPY Benchmark</button><small class="muted">Uses a persisted provider-attributed SPY quote; no regular-session-close claim.</small></article>` : "";
...
function renderDecision(state) {
  ...
  const executionControl = state.health.persisted && isExecutionEnabled(decision)
    ? `<article class="surface command-form"><span class="kicker">Execution readiness</span><h2>Ready for paper execution</h2><p class="muted">The backend will select the eligible persisted quote and calculate the simulated fill.</p><input id="execution-executed-at" type="datetime-local" aria-label="Execution timestamp" required><button class="refresh-button" id="execute-paper-trade" type="button">Execute Paper Trade</button></article>`
    : `<article class="surface"><span class="kicker">Execution readiness</span><h2>${esc(readiness.reasonCode)}</h2><p class="muted">Approval: ${esc(readiness.approvalStatus ?? "Not recorded")} · Validation: ${esc(readiness.validationStatus)}</p></article>`;
