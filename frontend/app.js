...
function normalizeReviewer(payload, context) {
  ...
  return {
    ...
    rationale: Object.hasOwn(payload, "rationale") ? nullableText(field(payload, "rationale", context), `${context}.rationale`) : null,
    reviewerId: Object.hasOwn(payload, "reviewer_id") ? nullableText(field(payload, "reviewer_id", context), `${context}.reviewer_id`) : null,
    findings: list(field(payload, "findings", context), `${context}.findings`).map((finding, index) => {
      ...
      return {
        ...
        whatWouldChange: Object.hasOwn(finding, "what_would_change") ? nullableText(field(finding, "what_would_change", item), `${item}.what_would_change`) : null,
      };
    }),
  };
}
...
function renderDecision(state) {
  ...
  const runControl = state.health.persisted && state.research?.decisionCycleId !== decision.decisionCycleId
    ...
    : "";
  const reviewerControl = state.health.persisted && !reviewer && decision.validation.status === "PASSED"
    ? `<article class="surface command-card"><span class="kicker">AI Reviewer</span><h2>Independent advisory review</h2><p class="muted">Reviews the exact research, recommendation, System Safety, and Manager Risk artifacts. It cannot approve, resize, or execute.</p><button class="refresh-button" id="run-reviewer" type="button">Run AI Reviewer</button></article>`
    : `<article class="surface command-card"><span class="kicker">AI Reviewer</span><h2>${esc(reviewer?.decision ?? "Not reviewed")}</h2>${reviewer ? `<p>${esc(reviewer.rationale ?? "No separate reviewer rationale was recorded.")}</p><ul class="decision-notes">${reviewer.findings.map((finding) => `<li><strong>${esc(finding.severity)} · ${esc(finding.category)}</strong><p>${esc(finding.message)}</p>${finding.whatWouldChange ? `<small class="muted">Would change: ${esc(finding.whatWouldChange)}</small>` : ""}</li>`).join("") || "<li>No reviewer findings.</li>"}</ul>` : `<p class="muted">No reviewer artifact is recorded for this decision.</p>`}</article>`;
...
export async function runReviewer({ decision, button = document.querySelector("#run-reviewer"), status = document.querySelector("#health-status"), reload = start } = {}) {
  button.disabled = true; button.textContent = "Running AI Reviewer…";
  try { await apiClient.post(`/commands/decisions/${encodeURIComponent(decision.decisionCycleId)}/review`); refreshStatus = "AI Reviewer result persisted."; await reload(); }
  catch (error) { refreshStatus = `AI Reviewer failed: ${error.message}`; status.textContent = refreshStatus; }
  finally { button.disabled = false; button.textContent = "Run AI Reviewer"; }
}
