...
from agentic_portfolio_lab.domain.research_provider import NormalizedEarningsFacts, NormalizedIncomeFacts, NormalizedOverviewFacts, ResearchProviderConfigurationError, ResearchProviderError, SourceResearchDocument, SourceResearchRecord
...
class AlphaVantageResearchProvider:
    """Normalizes OVERVIEW, INCOME_STATEMENT, and EARNINGS only."""
    ...
    def _earnings_record(self, payload: Mapping[str, object], security: SecurityIdentity) -> NormalizedEarningsFacts:
        ...
        report = cast(Mapping[str, object], reports[0])
        source = self._record("ALPHA_VANTAGE_EARNINGS", "Alpha Vantage quarterly earnings", self._date(report, "reportedDate", "EARNINGS"), {"reported_eps": report.get("reportedEPS"), "estimated_eps": report.get("estimatedEPS"), "surprise": report.get("surprise"), "surprise_percentage": report.get("surprisePercentage")})
        return NormalizedEarningsFacts(source, self._optional(report,"reportedDate"), self._optional(report,"fiscalDateEnding"), self._optional(report,"reportedEPS"), self._optional(report,"estimatedEPS"), self._optional(report,"surprise"), self._optional(report,"surprisePercentage"))
